 ...
 *         by a thread.
 */
public class TransferTransactionThreadSafe implements Runnable {
	private BankThreadSafe bank;
	private int fromAccount;

	public TransferTransactionThreadSafe(BankThreadSafe bank, int fromAccount) {
		this.bank = bank;
	...
	public void run() {
       //The controll will come here once a thread is started.
		while (true) {//It has been kept always true for continous execution
			int toAccount = (int) (Math.random() * BankThreadSafe.MAX_ACCOUNT);//Selecting traget account randomly.
			...
			int amount = (int) (Math.random() * BankThreadSafe.MAX_AMOUNT);//It will select a random amount less than or equal to 10
