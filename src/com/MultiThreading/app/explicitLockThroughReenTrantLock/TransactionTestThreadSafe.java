 ...
 * 
 */
public class TransactionTestThreadSafe {
    public static void main(String[] args) {
    	
        BankThreadSafe bank = new BankThreadSafe();//Created a sharable bank object for threads
        ...
        for (int fromAccountId = 0; fromAccountId < BankThreadSafe.MAX_ACCOUNT; fromAccountId++) {
        	//We have ten account and we are creating 10 threads. One thread for each account
            Thread t = new Thread(new TransferTransactionThreadSafe(bank, fromAccountId),"Thread Name -"+fromAccountId);            
